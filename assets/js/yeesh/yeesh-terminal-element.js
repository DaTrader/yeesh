...
function interceptKeys( event, host) {
  ...
  if (event.ctrlKey && event.shiftKey) {
    ...
    if (event.key === "P") {
      ...
      host.pushEventTo(host.el, "yeesh:history_prev", {
    ...
    if (event.key === "N") {
      ...
      if (host.historyPrefix === null)
        host.historyPrefix = host.inputBuffer;
      host.pushEventTo(host.el, "yeesh:history_next", {
        prefix: host.historyPrefix,
      });
      return false;
    }
  ...
  if (event.ctrlKey && event.key === "r") {
    ...
    if (!host.searchMode) {
      ...
      host.searchSkip++;
      host.pushEventTo(host.el, "yeesh:history_search", {
  ...
  if (host.searchMode) {
    ...
    if (event.key === "Enter") {
      ...
      if (input.trim().length > 0) {
        host.pushEventTo(host.el, "yeesh:input", { input });
      } else {
        host.writePrompt();
      }
      return false;
    ...
    if (event.ctrlKey && event.key === "c") {
      ...
      host.cursorPos = 0;
      host.historyPrefix = null;
      host.pushEventTo(host.el, "yeesh:interrupt", {});
      host.writePrompt();
      return false;
    }
